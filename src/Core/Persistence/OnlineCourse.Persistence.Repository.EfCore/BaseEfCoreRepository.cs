...
namespace OnlineCourse.Persistence.Repository.EfCore
{
         ...
         where TDbContext : CoreDbContext
    {
        ...
        {
            return await Entities.Where(filterExpression).FirstOrDefaultAsync(cancellationToken);
        }

        protected virtual IQueryable<TEntity> CreateFilteredQuery(TSearchEntity searchEntity)
        {
        ...
        public async Task<PagedResponse<IReadOnlyList<TEntity>>> GetAllAsync(TSearchEntity searchEntity, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                IQueryable<TEntity> query = CreateFilteredQuery(searchEntity);
                int countTask;
                int searchTask;
        ...
        {
            return await Entities.SingleOrDefaultAsync(p=>p.Id == id, cancellationToken);
        }
